...
# Build --------------------------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="AXI UART CORE")
    ...
    build_group.add_argument("--build_name",    default="axi_uart",             help="Build Folder Name")
    build_group.add_argument("--mod_name",      default="axi_uart_wrapper",     help="Module Name and File Name of the RTL")
    ...
    gen_path = os.path.join("axi_uart_gen.py")
    ...
    if args.build:
        # Build Path
        build_path = os.path.join(args.build_dir, 'ip_build/rapidsilicon/ip/axi_uart/v1_0/' + (args.mod_name))
