...
def main():
    ...
    core_fix_param_group.add_argument("--write_depth_B",    type=int,   default=1024,       choices=[64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144],  help="RAM Depth for Port B")
    ...
    if args.json:
        ...
        if (args.memory_type in ["Single_Port"]):
            option_strings_to_remove = ['--common_clk', '--write_width_B', '--read_width_B', '--write_depth_B', '--file_path']
            ...
            option_strings_to_remove = ['--write_width_B', '--read_width_A', '--write_depth_B', '--file_path']
            parser._actions = [action for action in parser._actions if action.option_strings and action.option_strings[0] not in option_strings_to_remove]
            # if (args.write_width_A != args.read_width_A):
                # option_strings_to_remove = ['--bram']
                # parser._actions = [action for action in parser._actions if action.option_strings and action.option_strings[0] not in option_strings_to_remove]
    ...
    if (args.write_depth_A > args.write_depth_B):
        memory_depth = args.write_depth_A
    else:
        memory_depth = args.write_depth_B
    
    if (args.memory_type == "True_Dual_Port"):
        if (args.write_width_A > args.write_width_B):
            memory_width = args.write_width_A
        else:
            memory_width = args.write_width_B
        ...
        if (args.write_width_A > args.read_width_B):
            ...
            memory_depth = args.write_depth_A
        # else:
        #     memory_width = args.read_width_B
        #     memory_depth = args.read_depth_A
        ...
        if (args.write_width_A > args.read_width_A):
            ...
            memory_width = args.read_width_A
            # memory_depth = args.read_depth_A
    
    # MEMORY_SIZE = 36*1024
    # if (args.read_width_B > args.write_width_A):# Read Wider
    #     if args.read_depth_B in [1024, 2048, 4096]:
    #         m = math.ceil(args.read_width_B/args.write_width_A)
    #         n = math.ceil((((args.write_depth_A*args.write_width_A)/36)/(1024*m)))
    #     elif args.read_depth_B in [8192, 16384, 32768]:
    #         n = math.ceil(args.write_width_A/9)
    #         m = math.ceil((((args.write_depth_A*args.write_width_A)/9)/(4096*n)))
    #     else:
    #         m = math.ceil(args.read_width_B/args.write_width_A)
    #         n = math.ceil((((args.write_depth_A*args.write_width_A)/36)/(args.read_depth_B*m)))
            
    # elif (args.write_width_A == args.read_width_B): # Symmetric
