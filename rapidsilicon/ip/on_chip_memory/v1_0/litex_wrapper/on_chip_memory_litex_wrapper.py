...
class OCM(Module):
    def __init__(self, platform, write_width_A, write_width_B, read_width_A, read_width_B, memory_type, common_clk, write_depth_A, read_depth_A, write_depth_B, read_depth_B, bram, file_path, file_extension):
        ...
        print("Write_A:", write_depth_A,"Write_B", write_depth_B)
        ...
        if (write_depth_A > 1024):
            ...
            if (memory_type != "Single_Port"):
                print("Else", msb_B, msb_A)
                self.addr_B_reg = Signal(msb_A - 10)
                
                # print(len(self.addr_B_reg))
                ...
                if (write_depth_A in [1024, 2048, 4096, 8192, 16384, 32768]):
                    ...
                    if write_depth_A > 1024:
                        ...
                        for i in range(m):
                            ...
                            for j in range(n):
                                # print(m, n)
                                self.comb += If((self.addr_A_reg == i), self.dout_B[(j*36):(j*36)+36].eq(Cat(self.bram_out_B[i][(j*32):(j*32)+16], self.rparity_B[i][(j*4):(j*4)+2], self.bram_out_B[i][(j*32)+16:(j*32)+32], self.rparity_B[i][(j*4)+2:(j*4)+4])))
                    # Need Attention !!!! Output from multiple BRAMs 
                    # ----------------------------------------------------
                    # self.data_out = [Signal(32*n) for i in range(m)]
                    # # self.data_out = Signal(36*n)
                    # for i in range(n):
                    #     print(i)
                    #     self.comb += self.data_out[0].eq(Cat(self.bram_out_B[i]))
                    #     self.comb += self.dout_B[0].eq(self.data_out[0])
                    # ----------------------------------------------------
