...
class AXICROSSBARWrapper(Module):
    def __init__(self, platform, m_count, s_count ,data_width, addr_width, s_id_width, aw_user_width, w_user_width, b_user_width, ar_user_width, r_user_width,
                aw_user_en, w_user_en, b_user_en, ar_user_en, r_user_en,bram, MADDRWIDTH):
...
def main():
    ...
    if args.json:
        ...
        if(num_elements == ((1*n) + 21)):

            for i in range (1, n+1):
               if((i + 21) <= num_elements):
                    arg_name = "address" + str(i) + "_Space"
    ...
    if args.build:
        ...
        wrapper = os.path.join(args.build_dir, "rapidsilicon", "ip", "axi_crossbar", "v2_0", args.build_name, "src",args.build_name + "_" + "v2_0" + ".v")
        new_lines = []
        with open (wrapper, "r") as file:
            lines = file.readlines()
            for i, line in enumerate(lines):
                if ("module {}".format(args.build_name)) in line:
                    new_lines.append("module {} #(\n\tparameter IP_TYPE \t\t= \"AXBR\",\n\tparameter IP_VERSION \t= {}, \n\tparameter IP_ID \t\t= {}\n)\n(".format(args.build_name, ip_version, ip_id))
                else:
