# AXI Async_FIFO IP Integration with VexRiscV CPU

## Design
This model consists of a VexRiscv CPU in an AXI4 implementation, connected to an AXI Block RAM as a peripheral via an AXI_Async_fifo. The CPU loads instructions from another AXI memory connected directly to the CPU instruction bus,as an embedded memory serving the purpose of a ROM.
...
## AXI Async FIFO
This serves as a multiclock domain crossing and a bridge between CPU and peripherals.
