...
class AHB2AXI4Wrapper(Module):
    def __init__(self, platform, data_width, addr_width, id_width):
       ...
       # self.comb += self.eq(platform.request("s_ahb_aclk"))
       # self.comb += self.eq(platform.request("s_ahb_aresetn"))
       ...
       # platform.add_extension(s_ahb.get_ios("s_ahb"))
       # self.comb += s_ahb.connect_to_pads(platform.request("s_ahb"), mode="slave")
