...
class TB(object):
    def __init__(self, dut):
        ...
        # control interface
        self.axil_master = AxiLiteMaster(AxiLiteBus.from_prefix(dut, "s_axil"), dut.clk, dut.rst,reset_active_level=False)
        # AXI interface
        self.axi_ram = AxiRam(AxiBus.from_prefix(dut, "m_axi"), dut.clk, dut.rst, size=2**16,reset_active_level=False)
...
async def run_test_write(dut, data_in=None, idle_inserter=None, backpressure_inserter=None):
    ...
    for length in list(range(1, byte_lanes*2))+[1024]:
        for offset in list(range(0, byte_lanes*2))+list(range(4096-byte_lanes, 4096)):
