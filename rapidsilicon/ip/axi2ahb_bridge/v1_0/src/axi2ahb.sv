...
//
//********************************************************************************
module axi4_to_ahb #(parameter TAG  = 1, Address_Width =32) (
...
output logic [31:0]     ahb_hwdata,      // ahb bus write data

input logic [31:0]      ahb_hrdata,      // ahb bus read data 
...
localparam ID   = 1;
localparam PRTY = 1;
typedef enum logic [3:0] {IDLE=4'b000, CMD_RD=4'b001, CMD_WR=4'b010, DATA_RD=4'b011, DATA_WR=4'b100, DONE=4'b101, STREAM_RD=4'b110, STREAM_ERR_RD=4'b111, WR_BURST=4'b1000, RD_BURST= 4'b1001} state_t;
...
logic [Address_Width-1:0] ahb_address;
logic                      address_valid;
...
logic                       last_addr_en;
logic [31:0]                last_bus_addr, last_ahb_addr;
...
function automatic logic [2:0] get_nxtbyte_ptr (logic [2:0] current_byte_ptr, logic [3:0] byteen, logic get_next);
   logic [2:0] start_ptr;
   logic       found;
   found = '0;
   //get_nxtbyte_ptr[2:0] = current_byte_ptr[2:0];
   start_ptr[2:0] = get_next ? (current_byte_ptr[2:0] + 3'd4) : current_byte_ptr[2:0];
...
function automatic logic [2:0] get_haddr_inc(logic hsize);
  logic [2:0] haddr_inc;
...
always_comb begin
   ...
   case (buf_state)
      IDLE: begin
               ...
               buf_cmd_byte_ptr[2:0] = buf_write_in ? get_nxtbyte_ptr(3'b0,buf_byteen_in[3:0],1'b0) : master_addr[2:0];
               bypass_en       = buf_state_en;
               rd_bypass_idle  = bypass_en & (buf_nxtstate == CMD_RD);
               ahb_htrans[1:0] = {2{bypass_en}} & 2'b10;
       end
      CMD_RD: begin
           buf_nxtstate = axi_arburst ? RD_BURST :(master_valid & (master_opc[2:0] == 3'b000))? STREAM_RD : DATA_RD;
//	          buf_nxtstate    = (master_valid & (master_opc[2:0] == 3'b000))? STREAM_RD : DATA_RD;  
      ...
      CMD_WR: begin
               master_ready   = 1'b1;
               buf_nxtstate = axi_awburst ? WR_BURST : DATA_WR;
               trxn_done    = ahb_hready_q & ahb_hwrite_q & (ahb_htrans_q[1:0] != 2'b0);
               buf_state_en = trxn_done;
               buf_cmd_byte_ptr_en = buf_state_en;
               ahb_address   = axi_awaddr;
              slvbuf_wr_en    = buf_state_en;
...
//                 ahb_htrans[1:0] = {2{~(cmd_done | cmd_doneQ)}} & 2'b10;
      ...
      WR_BURST: begin
//                 bypass_en    = 1'b1;
              buf_nxtstate = axi_wlast ? DONE : WR_BURST;
              buf_write_in = ~axi_wlast;
              trxn_done    = ahb_hready_q & ahb_hwrite_q & (ahb_htrans_q[1:0] != 2'b0);
              buf_state_en = trxn_done;
              buf_cmd_byte_ptr_en = buf_state_en; //get_haddr_inc(ahb_hsize)
              slvbuf_wr_en    = buf_state_en;
//                 buf_cmd_byte_ptr    = trxn_done ? get_nxtbyte_ptr(buf_cmd_byte_ptrQ[2:0],buf_byteen[7:0],1'b1) : 0;//buf_cmd_byte_ptrQ;

              ahb_address = address_valid ?   (last_ahb_addr + 4) : ahb_haddr;
              master_ready   = 1'b1;
              buf_cmd_byte_ptr    = get_haddr_inc(ahb_hsize); //buf_cmd_byte_ptrQ;
              cmd_done            = trxn_done & (buf_aligned | (buf_cmd_byte_ptrQ == 3'b111) | 
                                                 (buf_byteen[get_nxtbyte_ptr(buf_cmd_byte_ptrQ[2:0],buf_byteen[7:0],1'b1)] == 1'b0));
//                 ahb_htrans[1:0] = {2{~(cmd_done | cmd_doneQ)}} & 2'b10;
       ...
       RD_BURST: begin
        ...
        DATA_WR: begin
              ...
              ahb_htrans[1:0] = 2'b11;
               ...
               buf_cmd_byte_ptr = bypass_en ? get_nxtbyte_ptr(3'b0,buf_byteen_in[3:0],1'b0) : 
...
always @ (posedge clk or posedge rst_l) begin
   if (!rst_l) begin
     ...
     address_valid     <= 1'b0;
      ...
      if(axi_awburst== 2'b01 && axi_wlast == 1'b0) begin
          address_valid<=1'b1;
      end
      else begin
          address_valid<=1'b0;  
...
//  assign ahb_address = address_valid ?  (axi_awaddr + addr_offset) : ahb_address;
//assign ahb_address = address_valid ?  (axi_awaddr + 4) : ahb_haddr;
//assign ahb_address = address_valid ?  ahb_haddr : (axi_awaddr + 4) ;
...
assign buf_byteen_in[3:0]   = wrbuf_byteen[3:0];
...
assign ahb_hwrite      = bypass_en ? (master_opc[2:1] == 2'b01) : address_valid ? 1'b1 : buf_write;
...
rvdffs #(.WIDTH(32))   last_bus_addressff (.din(ahb_address[31:0]), .dout(last_ahb_addr[31:0]), .en(last_addr_en), .clk(ahbm_clk), .*);
...
rvdffs #(.WIDTH(4))               buf_byteenff  (.din(buf_byteen_in[3:0]), .dout(buf_byteen[3:0]), .en(buf_wr_en), .clk(buf_clk), .*);
